...
func TestScanDirectory(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			normalizedFiles := scanner.GetNormalizedPaths(result)
...
func TestPathNormalizer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := normalizer.BuildrootToInstalled(tt.buildrootFile, tt.buildrootPath)
...
func TestScanDirectoryErrorHandling(t *testing.T) {
	...
	t.Run("permission denied directory", func(t *testing.T) {
		...
		// Should not fail completely, but should log warnings and continue
		if err != nil {
			t.Errorf("Unexpected complete failure: %v", err)
		}

		// Should still find the buildroot directory itself
		if len(result.AllFiles) == 0 {
			t.Error("Expected to find at least some files despite permission errors")
		}
	})
}
...
func TestGetNormalizedPaths(t *testing.T) {
	...
	scanner := NewScanner(10, []string{})
	normalizedPaths := scanner.GetNormalizedPaths(result)
