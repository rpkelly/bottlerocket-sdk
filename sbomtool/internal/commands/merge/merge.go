...
func Merge(config MergeConfig, inputFiles []string) (*MergeResult, error) {
	...
	sboms, format, err := loadSBOMs(processor, inputFiles)
	if err != nil {
	...
	deduplicator := deduplication.NewDeduplicator(deduplication.DeduplicationConfig{
		PreferCPE:        true,
	...
	dedupResult := deduplicator.DeduplicatePackages(allPackages)

	// Phase 5: Relationship Processing
	updatedRelationships := deduplicator.UpdateRelationships(allRelationships, dedupResult.IDMapping)
	...
	if err := processor.SaveSBOM(finalSBOM, config.OutputPath, outputFormat); err != nil {
		return nil, fmt.Errorf("saving merged SBOM failed: %w", err)
	}
...
func loadSBOMs(processor *processor.SyftConfiguredProcessor, inputFiles []string) ([]*sbom.SBOM, string, error) {
	var sboms []*sbom.SBOM
