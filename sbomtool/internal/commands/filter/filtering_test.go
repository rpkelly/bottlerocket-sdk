...
func TestNewFilteringEngine(t *testing.T) {
	// GIVEN: No specific setup required
	// WHEN: NewFilteringEngine is called
	// THEN: A properly configured filtering engine should be returned

	engine := NewFilteringEngine()

	assert.NotNil(t, engine)
	assert.NotNil(t, engine.processor)
	assert.NotNil(t, engine.buildrootScanner)
}
...
func TestFilterSBOMByBuildroot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute filtering
			result, err := engine.FilterSBOMByBuildroot(inputSBOM, buildroot)
...
func TestFindPackagesWithBuildrootFiles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := engine.findPackagesWithBuildrootFiles(packages, tt.buildrootPaths)
...
func TestResolveTransitiveDependencies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := engine.resolveTransitiveDependencies(inputSBOM, rootPackages)
...
func TestIsDependencyRelationship(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := engine.isDependencyRelationship(tt.relType)
...
func TestValidateFilteredSBOM(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := engine.validateFilteredSBOM(testSBOM)
...
func TestSaveFilteredSBOM(t *testing.T) {
	// GIVEN: A filtering result and output parameters
	// WHEN: SaveFilteredSBOM is called
	// THEN: The SBOM should be saved successfully

	engine := NewFilteringEngine()
	testSBOM := createTestSBOM([]testPackage{
		{id: "pkg-a", name: "package-a", version: "1.0.0"},
	}, []testRelationship{})

	result := &FilteringResult{
		FilteredSBOM: testSBOM,
	}

	testDir := t.TempDir()
	outputPath := filepath.Join(testDir, "output.json")

	err := engine.SaveFilteredSBOM(result, outputPath, "syft-json")
